...
import { calcularStockCombo } from "@/lib/precio-dinamico";
  ...
  try {
    ...
    if (soloInventario === "true") {
      ...
      const productosInventario = productos.map(producto => {
        let stock = Number(producto.enStock);
        ...
        return {
          ...
          ultimaActualizacion: producto.movimientosInventario?.[0]?.fechaMovimiento?.toISOString() || producto.updatedAt.toISOString(),
          estado,
          sku: producto.sku,
    ...
    const productosTransformados = productos.map(producto => {
      let finalStock = Number(producto.enStock);
      if (producto.esCombo && producto.componentes) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    let fechasVencimientoValidadas: any = undefined;
    ...
    const productoData: any = {
      ...
      descripcion: descripcion && typeof descripcion === 'string' ? sanitizeInput(descripcion.trim()) : null,
      ...
      precioEspecial: precioEspecial ? Number(precioEspecial) : null,
      diasPrecioEspecial: diasPrecioEspecial ? JSON.stringify(diasPrecioEspecial) : null,
    };

    // Preparar relaciones, incluyendo creación dinámica de componentes si esCombo es true
    ...
    if (esCombo && componentes && Array.isArray(componentes) && componentes.length > 0) {
      productoData.componentes = {
        create: componentes.map((comp: any) => ({
          componenteId: comp.componenteId,
          cantidad: Number(comp.cantidad) || 1,
          esCortesia: comp.esCortesia ? true : false,
        })),
      };
      includeConfig.componentes = { include: { componente: true } };
    }
