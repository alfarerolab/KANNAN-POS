...
import { EstadoPagoFiado, type venta as VentaType } from "../../../../../lib/prisma-types";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const deudaTotal = ventasFiadas.reduce(
      (sum: number, venta: VentaType) => sum + Number(venta.saldoPendiente || 0),
      0
    );
    ...
    const resultado = await db.$transaction(async (prisma: typeof db) => {
      for (const venta of ventasFiadas as { id: string; saldoPendiente: unknown; montoPagado: unknown; fechaVencimiento: Date | null }[]) {
