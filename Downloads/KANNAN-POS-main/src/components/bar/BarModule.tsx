...
import type { ModoModulo } from "@/components/bar/context/BarContext";
import {
  ArrowRightLeft,
  ChefHat,
  ClipboardList,
...
import {
  ...
  METODOS_PAGO,
  type RolVista,
} from "@/components/bar/context/BarContext";
import { MeseroView } from "@/components/bar/views/MeseroView";
import { ComandaPreparacion } from "@/components/bar/ComandaPreparacion";
import { CuentaConsumo } from "@/components/bar/CuentaConsumo";
...
import { BarraCajaView } from "./views/BarraCajaView";
...
const ROLES: Array<{ id: RolVista; label: string; icon: typeof ChefHat }> = [
  ...
  { id: "cocina", label: "Barra / Caja", icon: ChefHat },
];

function RestauranteHeader() {
  const { recargandoVista, recargarOperativo, mesaSeleccionada, modo } = useBarContext();
  ...
  const borderColor = esBar ? "border-[#7c3aed]/20" : "border-[#f59e0b]/20";
  const BadgeIcon = esBar ? Wine : Sparkles;
  return (
    <div className={`rounded-2xl sm:rounded-[28px] border ${borderColor} bg-card p-4 sm:p-6`}>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="space-y-1">
          ...
          <h1 className="text-lg sm:text-2xl font-semibold tracking-tight text-foreground">
            {esBar ? "Operación de barra" : "Operación de mesas"}
          </h1>
        </div>
...
function DialogsGlobales() {
  const {
    ...
    mesaDestinoId, setMesaDestinoId,
    mesasParaDesunir, setMesasParaDesunir,
    divisionPersonas, setDivisionPersonas,
    ...
    agregarProductoHandler, guardarItemHandler, unirMesasHandler,
    desunirMesaHandler,
  ...
  // Resetear mesasParaDesunir cuando se cierra el dialog
  useEffect(() => {
    if (!unirMesasOpen) {
      setMesasParaDesunir([]);
    }
  }, [unirMesasOpen, setMesasParaDesunir]);
  ...
  // Mesas unidas al pedido actual (excluye la mesa principal)
  ...
  return (
    <>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={productoForm.estacion} onValueChange={(v) => setProductoForm((p) => ({ ...p, estacion: v as EstacionPreparacionRestaurante }))}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>{ESTACIONES_PREPARACION.filter(e => e !== "COCINA").map((e) => (<SelectItem key={e} value={e}>{getEstacionPreparacionLabel(e)}</SelectItem>))}</SelectContent>
                  </Select>
                </div>
              </div>
      ...
      {/* ── Dialog Unir / Desunir Mesas ───────────────────────────────────────── */}
      <Dialog open={unirMesasOpen} onOpenChange={setUnirMesasOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Mesas de la cuenta</DialogTitle>
            <DialogDescription>
              {pedidoActivo
                ? "Une mesas libres a esta cuenta o desune las que ya están vinculadas."
                : "Selecciona las mesas que quieres unir. Se abrirá una cuenta compartida cuando se agregue el primer producto."}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-5">
            ...
            <div className="space-y-3">
              <p className="text-sm font-semibold text-foreground">Unir mesas a esta cuenta</p>
                ...
                mesasDisponiblesParaUnir.map((mesa) => {
                  ...
                  return (
                    <button
                      key={mesa.id}
                      type="button"
                      className={`flex w-full items-center justify-between rounded-2xl border p-4 text-left transition ${checked ? "border-orange-500/40 bg-orange-500/10" : "border-border hover:border-border"}`}
                      onClick={() => setMesasParaUnir((prev) => prev.includes(mesa.id) ? prev.filter((id) => id !== mesa.id) : [...prev, mesa.id])}
                    >
                      <div>
                        <p className="font-medium text-foreground">{mesa.nombre}</p>
                        <p className="text-sm text-muted-foreground">
                          {mesa.pedidoAbierto ? `Consumo: ${formatCurrency(mesa.pedidoAbierto.total)}` : "Mesa libre sin consumo"}
                        </p>
                      </div>
              ...
              <Button
                className="w-full"
                disabled={mesasParaUnir.length === 0 || uniendoMesas}
                onClick={unirMesasHandler}
              >
                {uniendoMesas ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Users className="mr-2 h-4 w-4" />}
                Consolidar mesas seleccionadas
              </Button>
            </div>

            {/* ── Sección: Desunir mesas — solo si hay mesas unidas ── */}
            {mesasUnidas.length > 0 && (
              <div className="rounded-2xl border border-dashed border-destructive/40 p-4 space-y-3">
                <div>
                  <p className="text-sm font-semibold text-destructive">Desunir mesas</p>
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Selecciona una o varias mesas para separarlas de esta cuenta.
                  </p>
                </div>

                <div className="space-y-2">
                  {mesasUnidas.map((mp) => {
                    const isSelected = mesasParaDesunir.includes(mp.mesa.id);
                    return (
                      <button
                        key={mp.mesa.id}
                        type="button"
                        ...
                        onClick={() =>
                          setMesasParaDesunir((prev) =>
                            prev.includes(mp.mesa.id)
                              ? prev.filter((id) => id !== mp.mesa.id)
                              : [...prev, mp.mesa.id]
                          )
                        }
                      >
                        <p className="text-sm font-medium text-foreground">{mp.mesa.nombre}</p>
                        <Badge variant={isSelected ? "destructive" : "secondary"}>
                          {isSelected ? "Seleccionada" : "Unida"}
                        </Badge>
                      </button>
                    );
                  })}
                ...
                <Button
                  variant="destructive"
                  className="w-full"
                  disabled={mesasParaDesunir.length === 0 || uniendoMesas}
                  onClick={() => desunirMesaHandler(mesasParaDesunir)}
                >
                  {uniendoMesas
                    ? <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                    : <Unlink className="mr-2 h-4 w-4" />
                  }
                  {mesasParaDesunir.length > 1
                    ? `Desunir ${mesasParaDesunir.length} mesas`
                    : "Confirmar desunión"}
                </Button>
              </div>
            )}
      ...
      {ticketData && (<TicketPrinter ticketData={ticketData} open={ticketPrinterOpen} onOpenChange={setTicketPrinterOpen} onPrintComplete={() => setTicketPrinterOpen(false)} autoImprimir={false} />)}
    </>
  );
}

function RestauranteContenido() {
  const { cargandoVista, rolVista, setCrearMesaOpen } = useBarContext();
  if (cargandoVista) {
    return (
  ...
  return (
    <div className="space-y-6">
      <RestauranteHeader />
      {rolVista === "mesero" && (<><div className="flex justify-end"><Button size="sm" className="bg-foreground text-background hover:bg-foreground/90" onClick={() => setCrearMesaOpen(true)}><Plus className="mr-1.5 h-3.5 w-3.5" />Nueva mesa</Button></div><MeseroView /></>)}
      {rolVista === "cocina" && <BarraCajaView />}
      <DialogsGlobales />
    </div>
  );
}

export function BarModule({ defaultView, modo = "restaurante" }: { defaultView?: "mesero" | "cocina"; modo?: ModoModulo }) {
