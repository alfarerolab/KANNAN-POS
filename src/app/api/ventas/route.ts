...
export async function GET(request: NextRequest) {
  try {
    ...
    if (fechaInicio || fechaFin) {
      ...
      if (fechaFin) {
        whereClause.createdAt.lte = new Date(fechaFin);
      }
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const {
      ...
      diasCredito,
      fechaVencimiento,
      consumosInternos, // NUEVO: productos usados internamente durante servicios
    } = body;
    ...
    for (const item of items) {
      ...
      if (!productoId || !cantidad || cantidad <= 0) {
        return NextResponse.json(
          {
            mensaje:
              "Cada ítem debe tener un ID de producto y una cantidad válida",
          },
          { status: 400 }
        );
      }

      // Obtener detalles del producto con componentes de combo
      ...
      // Acumular cantidades a descontar (expandiendo combos)
    ...
    // ─── Descontar stock de Consumos Internos (sin cobrar) ───────────────
    if (consumosInternos && Array.isArray(consumosInternos)) {
      for (const consumo of consumosInternos) {
        if (consumo.productoId && consumo.cantidad > 0) {
          cantidadesPorProducto.set(
            consumo.productoId,
            (cantidadesPorProducto.get(consumo.productoId) ?? 0) + Number(consumo.cantidad)
          );
        }
      }
    }
    ...
    for (const producto of productosADescontar) {
      ...
      if (Number(producto.enStock) < cantidadSolicitada) {
        return NextResponse.json(
          {
            mensaje: `Stock insuficiente para ${producto.nombre}. Disponible: ${producto.enStock}, Solicitado: ${cantidadSolicitada}`,
          },
          { status: 400 }
        );
      }
    ...
    const nuevaVenta = await db.$transaction(async (prisma) => {
      ...
      if (esVentaFiada) {
        ...
        if (diasCredito) {
          ...
          fechaVencimientoVenta.setDate(
            fechaVencimientoVenta.getDate() + diasCreditoVenta
          );
          ...
          fechaVencimientoVenta = new Date();
          fechaVencimientoVenta.setDate(
            fechaVencimientoVenta.getDate() + 30
          );
        }
      }
      ...
      const venta = await prisma.venta.create({
        data: {
          ...
          usuarioId,
          clienteId,
          notas: (notas ? sanitizeInput(notas) : "") + notasConsumo || null,
          esVentaFiada,
          ...(esVentaFiada && {
            saldoPendiente,
      ...
      for (const producto of productosADescontar) {
        ...
        await prisma.producto.update({
          ...
          data: { enStock: { decrement: cantidadVendida } },
        });
      }

      // 3.5 Descontar stock de consumos internos
      if (consumosInternos && Array.isArray(consumosInternos) && consumosInternos.length > 0) {
        for (const consumo of consumosInternos) {
          if (consumo.productoId && consumo.cantidad) {
            await prisma.producto.update({
              where: { id: consumo.productoId },
              data: { enStock: { decrement: Number(consumo.cantidad) } },
            });
            // Registrar movimiento de inventario si se desea, pero con descontar stock ya afecta Inventario
          }
        }
      }
